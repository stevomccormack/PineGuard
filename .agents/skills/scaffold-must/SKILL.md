---
name: scaffold-must
description: Implement a new MustClause fluent validation method. Use when adding Must.Be.X validation — whenever the user says "add a Must clause", "implement Must.Be.Xxx", "add a fluent validation method", or needs a result-returning validation method that never throws. Do NOT use for Core rules, Guard clauses, Fluent extensions, or DataAnnotations.
