<#
.SYNOPSIS
    Unlists prerelease versions of PineGuard packages on nuget.org.

.DESCRIPTION
    nuget.org has no true delete — this sends unlist requests via
    ...
    Pass -All to unlist every prerelease including the latest one.
    Stable versions (no -alpha/-beta/-rc suffix) are never touched.

.PARAMETER Package
    Limit to specific package IDs. Defaults to all six PineGuard packages.
...
.EXAMPLE
    pwsh -File .etc/powershell/unlist-prereleases.ps1 -DryRun

.EXAMPLE
    pwsh -File .etc/powershell/unlist-prereleases.ps1 -All -Force
#>
...
foreach ($line in Get-Content $envFile) {
    if ($line -match "^\s*NUGET_TOKEN\s*=\s*'([^']+)'") {
        $token = $Matches[1]
        break
    }
}
if (-not $token) { Fail "NUGET_TOKEN not found in $envFile" }
...
foreach ($pkg in $Package) {
    ...
    # Flat-container returns ascending order; the last entry is the latest version.
