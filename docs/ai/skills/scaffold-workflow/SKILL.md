...
3.  **Cascade to every surface** — produce or explicitly N/A each row:
    ...
    | 8 | `.github/copilot-instructions.md` | Palette row under the matching `###` section | release family (§4) |
    | 9 | `.github/prompts/<name>.prompt.md` | Copilot prompt pointing at the playbook | release family (§4) |
    | 10 | `.opencode/commands/<name>.md` | `Act as **<Role>**. Read and execute docs/ai/agents/<name>.md.` (same one-liner as `.claude/commands/`, plus a `description:` frontmatter) | release family (§4) |
    | 11 | `.agents/skills/<name>/SKILL.md`, `.github/skills/<name>/SKILL.md`, `.codex/agents/<name>.toml` (§2.1) | One `SKILL.md` per Brain skill; TOML per Codex agent | ordinary agent changes; only touch these if the change adds or renames a **skill or subagent** |
    | 12 | Rules-only adapters (§3) | — | ordinary agent changes; only touch these if the change alters a **layer mapping** |
    | 13 | `.vscode/tasks.json` | Task entry | the agent has no task-runner equivalent |
