...
- `docs/ai/specs/tools/audit-cli/spec.md`, `tools/audit-cli/README.md`: `must-codes`.
...
4. Write `Test-`must-codes`-MustCodes.ps1` in `tools/audit-cli/rules/`, register in `Load-Catalog.ps1`, `Run-AuditLibraryRules.ps1`, `.vscode/tasks.json`; document in `tools/audit-cli/README.md` and `docs/ai/specs/tools/audit-cli/spec.md`.
5. `pwsh -NoProfile -ExecutionPolicy Bypass -File "<wt>/tools/audit-cli/Run-All.ps1" -Configuration Release -RuleId `must-codes`,`layer-parity`,`ordering`,`test-files`` → clean. Break one clause, confirm `must-codes` fails, revert.
