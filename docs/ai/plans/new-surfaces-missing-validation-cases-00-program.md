...
| Audit rules | the legacy `tools/audit-cli` tool's `Run-All.ps1 -Configuration Release -RuleId <ids>`; rule scripts in `rules/Test-Rule*.ps1`; catalogue in its `README.md` | Rule ids used in this program: 03, 06, 07, 08, 10, 11, 12, 13 (new), 14 (new), 50, 53 |
| Format / Roslyn / Qodana | `tools/code-format/Run-Format.ps1`, `tools/code-diagnostics/Run-CompilerDiagnostics.ps1`, `tools/code-scan/qodana/Run-Qodana.ps1` | All take `-Scope` |
| Structural checks | `tools/clean/Test-StructuralIntegrity.ps1` (namespace = folder, one type per file, …) | Run before every PR |
...
| Where planned files are written | `+ src/...` inside code spans (this file §1, above) | `doc-links` audits every other backticked path |
...
- **Namespace equals package id**, and folders inside the project match namespaces. `tools/clean/Test-StructuralIntegrity.ps1` checks namespace/folder alignment, so the Microsoft habit of putting `IServiceCollection` extensions into the `Microsoft.Extensions.DependencyInjection` namespace is deliberately **not** followed — consumers add one `using PineGuard.<Framework>;`.
...
8. The catalogue mirrors the path — `MustCodes.Email.Address.Invalid` ↔ `"email.address.invalid"` — structurally: values are composed constants (`Prefix + ".invalid"`, folded at compile time). Audit `must-codes` verifies the textual invariants (every clause passes a constant; no code string literal outside `src/PineGuard.Core/Codes/`) and the reflection test `MustCodesTests` verifies grammar shape, mirroring, uniqueness and `<summary>` agreement.
...
# 7. Pull request; wait for CI (build, test matrix, coverage 100%, format, roslyn, audit `test-files`)
...
- [ ] **Tests**: the §9 test command (`Run-Tests.ps1 -Solution ./PineGuard.slnx -Configuration Release`) green on `net8.0` and `net10.0`; no `[Fact]`, no `[InlineData]`; every `XxxTests.cs` has its `XxxTestData.cs` (`test-files`, the one audit rule CI enforces).
...
- [ ] **Audit**: the `test-files` rule clean (CI gate); `layer-parity`, `ordering` clean for any phase that adds Must clauses; the `must-codes` rule clean from Phase 1 onward; Rule14 (Core stays synchronous) clean from Phase 3 onward; `doc-links`/`surface-parity` clean for any phase that adds agents or docs; dropped rules `Rule09`/`Rule10` have no equivalent in the new audit.
...
- [ ] **Adapters**: every new agent cascaded per `docs/ai/meta/adapter-surfaces.md` §5; `surface-parity` clean.
...
| 9 | `tools/code-coverage/New-CoverageReport.ps1` | `ValidateSet`; source-dir switch; expected-path regex switch; include-pattern switch (`[PineGuard.<Fw>]*`); default project filter switch; the `default` (All) regex and include list |
| 10 | `tools/code-coverage/Test-Coverage.ps1` | `ValidateSet`; source-dir switch; `IncludeFileRegex` switch; the `All` regex |
| 11 | `tools/.shared/coverage.ps1` line 215 | `$prefixes` list |
| 12 | `tools/code-format/Run-Format.ps1` | `ValidateSet` + project switch |
| 13 | `tools/code-diagnostics/Run-CompilerDiagnostics.ps1` | `ValidateSet` + project map |
| 14 | `tools/code-scan/qodana/Run-Qodana.ps1` + a new `qodana.<scope>.yaml` in `tools/code-scan/qodana/config/` + a new `PineGuard.<Fw>.Qodana.slnx` beside the existing per-scope ones | `ValidateSet`, config switch, slug switch |
| 15 | the former Qodana coverage wrapper (`Run-Coverage` under the old code-inspection folder) | Nothing to edit: retired in the 2026-09 tools standardisation |
| 16 | `tools/git/Run-Commits.ps1` | Nothing to edit: `-Scope` is validated against the registry, so a new registry scope is accepted automatically |
| 17 | `tools/git/Run-Commits.ps1` (the `Agent` meta-scope path list) | Add `src/PineGuard.<Fw>/AGENTS.md` to the agent-file list |
| 18 | `tools/github/Run-Release.ps1` | Nothing to edit: the package list derives from the registry via `Get-PineGuardPackableProjects` |
| 19 | `tools/nuget/Unpublish-NugetPrerelease.ps1` | Nothing to edit: same registry-derived package list |
| 20 | `tools/audit-cli/rules/Test-`test-orphans`-TestOrphans.ps1` | Nothing to edit: it derives `src/<TestProject minus .UnitTests>` and skips projects whose source folder is absent. Consequences: a normal scope resolves automatically; `PineGuard.Analyzers.UnitTests` maps to `+ src/PineGuard.Analyzers` only, so code-fix tests live inside the analyzer test classes (Plan 06 §3.2); `PineGuard.Testing.UnitTests` is skipped entirely. `test-orphans` is a local convention check, not a CI gate |
| 21 | `.vscode/tasks.json` | `Test: <Fw> (fast)`, `Coverage: <Fw> (fast)`, `Format: <Fw>`, `Git: Commit <Fw> (auto message)`, `Inspect: Qodana: <Fw>` |
| 21a | `tools/audit-cli/rules/Test-`must-codes`-MustCodes.ps1` | The hardcoded `$extraRoot` array (currently `src/PineGuard.Core`, `src/PineGuard.DataAnnotations`, `src/PineGuard.AspNetCore`) that `must-codes` check (b) scans for MustCodes constant usage — add `src/PineGuard.<Fw>` when the new scope carries `Must.Be.*` call sites of its own. Missing from the original checklist; found and fixed during Plan 02's Options fix-and-reverify pass (2026-08-29) |
...
| Audit | `test-files` gate; also `layer-parity`, `ordering`, `must-codes` when clauses change; `doc-links`, `surface-parity` when docs/agents change | `docs/ai/agents/audit-cli.md` |
...
|---|---|---|---|
| **Track 0** | The `Get-PineGuardScope` registry in `tools/.shared/dotnet-projects.ps1` + the refactor of §8.3 items 8–15 to read it + the §8.2 CI changes (7e–7g, and 7b if approved) + a `must-codes`-style `Codes/` purity check is **not** here (it is `must-codes` (g), Plan 01). Creates **no** `src/` or `tests/` project — the first scope (Options) is onboarded by Plan 02 using the registry. **Current state**: approved, running in Wave 0 (§12). | — | — (touches `tools/`, `.github/`, `.vscode/tasks.json` only; must not touch `tools/audit-cli/`, which 1a owns) |
| **1a** | Phase 1 W1–W3: `IMustResult`, codes migration + `must-codes`, `MustValidationResult`, `MustValidator<T>` keystone, test family | — | no |
...
| **5-A … 5-F** | Six rule batches, each its own PR | 1a (codes + `must-codes`); 5-E also adds `FixedTimeProvider` to `PineGuard.Testing` | no |
...
- The known **union-merge set** — files every scope-adding unit touches and that will conflict trivially: `PineGuard.slnx`, `Directory.Packages.props`, `.editorconfig` (line 520 list), `.github/workflows/ci.yml`, `tools/git/Run-Commits.ps1`, `.vscode/tasks.json`, `CLAUDE.md`, root `AGENTS.md`, `GEMINI.md`, the since-retired Pi palette, root `README.md`, `docs/ai/README.md`, `docs/ai/specs/testing/gold-standard.md`, `docs/ai/specs/testing/coverage.md`, `docs/ai/meta/taxonomy.md`, `.github/dependabot.yml`, `sonar-project.properties`, `tools/.shared/dotnet-projects.ps1`, the legacy `tools/audit-cli` tool's `Load-Catalog.ps1`, plus for clause-adding units the `src/PineGuard.Core/Codes/MustCodes.<Domain>.cs` file of any domain two units both extend (the per-domain split keeps most batches conflict-free) and `docs/ai/specs/language/vocabulary.json`. Resolve by taking both sides; `git merge origin/main` into the branch before opening the PR; never rebase a pushed branch.
...
- Public validator assertion helpers in `PineGuard.Testing` (roadmap Part 8) — the package is **already published** (`tools/github/Run-Release.ps1`), so the `MustValidationResult` test family and `FixedTimeProvider` added here ship in it; a consumer-facing assertion API (`ShouldHaveFailure(...)`) is a later plan.
...
| 2026-08-26 | Error codes are fixed three-segment addresses `<domain>.<aspect>.<condition>`, curated, never derived from method names | The mechanical `email.email` scheme doubled segments and read poorly; codes must be XPath-like with every segment load-bearing and the domain always present | §5.4; Plan 01 §4.1 (curation checkpoint; `must-codes` = textual usage checks, `MustCodesTests` = shape, mirroring, uniqueness) |
...
| 2026-08-26 | `must-codes` ships in stages with the PR that adds the code it audits (1a: clauses; 1c: attributes; 1d: guards) | Shipping every check in 1a would leave `main` red for all of Wave 1 | Plan 01 §4.1, §7 |
...
| 2026-08-29 | `-Scope All` aggregate include-regexes derive from the registry's path fields (`PathIncludeRegex` joined with `\|` in Test-CoverageAnalysis; regex-escaped `SourceDir` leaves in Gen-CoverageReport), never from `Name` — and no `FolderLeaf`-style field is added to the registry | Joining `Name` as `PineGuard\.(<names>)` breaks for any scope whose folder is not `PineGuard.<Name>` (Options → `PineGuard.Extensions.Options`), silently skipping its files at the §7 `-Scope All -Enforce100` gate; each script's fix mirrors its own per-scope branch (single source of truth, zero registry edits, new scopes correct automatically); a duplicate folder field would touch all seven entries and could drift from `SourceDir`/`PathIncludeRegex`. **Correction (still 2026-08-29):** the `Gen-CoverageReport.ps1` half is currently defensive-only, not load-bearing — inside `Test-CoverageLooksValid`, `$expected`'s sole consumer (the mismatch check that would `Write-Warning` and `return $false`) sits inside a pre-existing `if ($false) { … }` block that was already on `main` before this change; `$expected` itself is still computed unconditionally, but nothing currently acts on it. So today only the `Test-CoverageAnalysis.ps1` half actually guards the live `-Scope All -Enforce100` gate; the `Gen-CoverageReport.ps1` fix is correct and ready for the day that block is revisited, but that re-enablement is a separate, later decision, not made here | `tools/code-coverage/Test-Coverage.ps1` (`'All'` branch), `tools/code-coverage/New-CoverageReport.ps1` (aggregate `$expected` branch) |
| 2026-09-06 | Final-review wire-up family closed with none added; the one identified gap, the coded DataAnnotations runner, built as `DataAnnotationsAttributeValidator.Validate()` in `PineGuard.DataAnnotations` | `AddGuardClauses()`/`UseGuardClauses()`/`AddDataAnnotations()` all rejected per the existing 2026-08-26 assessment below — nothing changed that assessment. Name chosen over `DataAnnotationsValidator` (simple-name collision with Blazor's `Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator`) and `MustValidationResult.FromAnnotations` (would require a `Core → DataAnnotations` circular project reference to read `ValidationAttributeBase.Code`, the entire reason the method would exist) | §5, §13.4 (orchestration.md D5); `src/PineGuard.DataAnnotations/DataAnnotationsAttributeValidator.cs`; `PineGuard.DataAnnotations.UnitTests` |
...
**Resolved 2026-09-06 (owner sign-off).** Wire-up family closed with none added — the assessment above stands unchanged. The coded runner gap was built: `DataAnnotationsAttributeValidator.Validate(object, IServiceProvider?)` in `src/PineGuard.DataAnnotations/DataAnnotationsAttributeValidator.cs`, named over the other two candidates — `DataAnnotationsValidator` collides by simple name with Blazor's `Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator`; `MustValidationResult.FromAnnotations` would need `PineGuard.Core` to reference `PineGuard.DataAnnotations` (to read `ValidationAttributeBase.Code`), which already references `PineGuard.Core` — a circular project reference, and the entire reason the method would exist. Runs every `ValidationAttribute` on an object's type and public properties independently (does not stop at the first failure per property, unlike `Validator.TryValidateProperty`); every failure from a `ValidationAttributeBase` carries its `Code`, every other failure carries `Code = ""`. `PineGuard.DataAnnotations.UnitTests` at 100/100 line+branch. The `ValidationResult.ToMustValidationResult()` bridges were already shipped at D4 (2026-08-30, `PineGuard.FluentValidation`).
...
| **Codes are unreachable at runtime on every *framework-invoked* path** (`Validator`, MVC, `ValidateDataAnnotations()`, Blazor all return code-less `ValidationResult`s) | documented limitation — no slot in `ValidationResult`, not fixable | the coded runner below fixes the manual path |
| Shallow coded runner returning `MustValidationResult` with each attribute's `Code` | **done — 2026-09-06** | `DataAnnotationsAttributeValidator.Validate()`; named over `DataAnnotationsValidator` (Blazor component collision) and `MustValidationResult.FromAnnotations` (circular project reference) |
...
| DataAnnotations | `attribute.Code` (design-time); `DataAnnotationsAttributeValidator.Validate()` (manual runtime path); every *framework-invoked* path (`Validator`, MVC, `ValidateDataAnnotations()`, Blazor) still returns code-less `ValidationResult`s | yes, via the manual path (§13.4) |
