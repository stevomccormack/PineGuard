...
| 7b | `.github/workflows/ci.yml` coverage threshold step | **Owner decision (§12, open)**: CI already hard-gates on `vars.MIN_CODE_COVERAGE` (= 90) via an explicit "Fail if coverage check failed" step; `continue-on-error: true` on the check step is not gate-softening — it only defers that failure so the sticky PR comment can post first, and the following step still fails the job. The 100/100 rule in §7 is enforced locally by `-Enforce100` and is not yet the CI-enforced minimum. Track 0 (merged 2026-08-29) is the prerequisite that makes raising it safe: its per-scope assembly filter stops a partial-scope PR from false-failing on assemblies it never touched. Raising `vars.MIN_CODE_COVERAGE` to 100 remains open for the owner; `continue-on-error` does not need to change either way |
...
| Coverage 100/100 | `pwsh -NoProfile -ExecutionPolicy Bypass -File "./tools/code-coverage/Run-CodeCoverage.ps1" -Mode GenerateAndAnalyze -Scope <Scope> -SkipHtml -Enforce100 -Framework net10.0` then `… -Framework net8.0` (one TFM per run — see §7). CI gate: hard-fails at `vars.MIN_CODE_COVERAGE` = 90 today (see corrected §8 item 7b — `continue-on-error` does not soften this); raising the threshold to 100 is the still-open §12 item | `docs/ai/specs/testing/coverage.md` |
...
Still open (owner's call; none blocks Wave 0): (1) Phase 4 bridges — all three vs ErrorOr-only (fallback in §3.3); (2) §5.4 vocabulary — `blank` vs the literal `null-or-white-space`; (3) `ValidationOptions.AddMustValidators()` vs the council's `AddMustValidatorResolver()` (§5.3) — decide before 3-PR2; (4) CI coverage threshold to 100 (§8 item 7b) — Track 0 has merged, so this is no longer a "before Track 0" gate, just an open owner call on raising `vars.MIN_CODE_COVERAGE`; `continue-on-error` does not need to change either way (see corrected §8 item 7b); (5) the §13 **adopt** proposals (`FluentMustValidator<T>` and FV auto-validation, `SetMustValidator`, the result bridges, `ToValidationResults()`, the coded DataAnnotations runner, `MustValidator<T>.Rules`) — the FV-shop migration story depends on the first of these, so decide before 1b starts.
