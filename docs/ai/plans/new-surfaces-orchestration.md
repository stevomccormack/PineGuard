...
| **3-PR2 — AspNetCore** | **In progress — pipeline integration surface, its full test coverage (126 tests net10.0 / 104 net8.0), and W7's `ValidationOptions.AddMustValidatorResolver()` (D3) all committed and independently verified (0 warnings/errors both TFMs).** Remaining: coverage gate confirmation on the newly-landed resolver code, and the TestServer end-to-end integration suite (this unit's largest remaining test surface). | `feature/aspnetcore` (open) | — | Independent verification caught and fixed 3 real bugs a prior dispatch's "builds clean" claim missed (a missing test class, 2 real assertion failures) — see §7 for the lesson this reinforced: always independently run tests, not just build, before trusting a dispatch. |
...
| **5-E** (temporal + clock injection) | **In progress — Core and Must/Guard layers essentially complete.** Core clock injection fully done. Must layer: `MinimumAge` clause + full `TimeProvider?` sweep across all five temporal families + calendar predicates (`Weekday`/`Weekend`/`FirstDayOfMonth`/`LastDayOfMonth`) on DateOnly/DateTimeOffset — all tested (`b5b2641`…`ef00481`, MustClauses 3026/3026 both TFMs). Guard layer: `TimeProvider?` threading + `BelowMinimumAge` on DateOnly/DateTime/string-DateOnly, all tested (`dca298f`, `5dd63e7`, `d9f65c1`, GuardClauses 2637/2637 both TFMs); Guard-layer calendar predicates for DateOnly/DateTimeOffset in flight. Remaining: the entire Fluent and DataAnnotations layers (DA resolves `TimeProvider` from `ValidationContext`); full Core/MustClauses/GuardClauses coverage gate. `FixedTimeProvider` clock test double shipped in `tests/PineGuard.Testing/`. | `feature/rules-temporal` (open) | — | Two decisions applied per the 2026-08-30 finish-everything directive rather than paused on: **(1)** `Microsoft.Bcl.TimeProvider` 10.0.11 added as a `Directory.Packages.props` dependency (netstandard2.1-only `PackageReference` on `PineGuard.Core`) to unblock `DateTimeRules`/`DateTimeOffsetRules`/their string counterparts, which otherwise can't reference `TimeProvider` pre-net8 — official Microsoft BCL polyfill, same tier as this program's other Microsoft.Extensions.* pins, reversible if the owner objects. **(2)** Plan 05's minimum-age row contradicts itself (states `today.AddYears(-years) >= value` as the boundary, then glosses it with leap-day behavior that formula doesn't actually produce — `value.AddYears(+years) <= today` does). Shipped the literal formula (matches .NET's dominant age-calculation idiom, is the conservative reading for a permission gate), documented the actual 1-March-not-28-Feb consequence in the `HasMinimumAge` XML `<remarks>`, pinned with a named test. Both are one-line-reversible if the owner prefers otherwise — flag before Must/Guard/Fluent/DataAnnotations build on the semantic. |
...
| **6 — Analyzers** | **In progress — all four planned PG1xxx diagnostics implemented and tested.** `PineGuard.Analyzers`/`PineGuard.Analyzers.CodeFixes` (netstandard2.0) onboarded into the solution/CI. PG1001 (`Guard.Against.Null`), PG1002 (`NullOrWhiteSpace`), PG1003 (`NullOrEmpty`), PG1004 (`OutOfRange`) all done end to end — analyzer, code fix, fix-all, tests (`b2fb9d6`…`4f51a2d`, 85/85 tests both TFMs). Shared coverage-scope registry (`tools/.shared/dotnet-projects.ps1`) generalized to support a two-source-project scope and wired in (`6fba593`) — surfaced a real coverage gap (99.69%/96.09%) now being closed. Remaining: coverage gate to 100%/100%, PG2001/PG2002 (discarded-`MustResult`/`MustValidationResult` diagnostics — a structurally different fire condition, not started), README before/afters, `.nupkg` layout verification. | `feature/analyzers` (open) | — | Deliberately run last (Plan 00 §3.2); no blockers. |
...
| D5 | Final-review wire-up family (`AddGuardClauses()` etc.) | program close | Current Plan 00 §12 assessment looks right; nothing in Phases 2–6 needs it earlier | **Owner sign-off required — not a council call**, by design. Not superseded by the 2026-08-30 directive: it names this row specifically as `D5-class`, so it stands until program close regardless. |
