...
> signal.** Its dependencies are kept patched when they block a build, but its `layer-parity`/07/08/11/13/50
...
1. `docs/ai/specs/testing/unit-test.md` — `[Theory]` + `TheoryData`/`[MemberData]` only (never
   `[Fact]` or `[InlineData]`; CI-gated by audit-cli `test-files`, §11); `XxxTests.cs` beside
   `XxxTestData.cs` (also `test-files`-gated); the per-layer `*Expected`/`*Case` record shapes (§2.2,
...
| Situation | Route |
|---|---|
| Name fully determined by existing convention — a new Must clause beside ~550 precedents, test/fixture file names (`test-files`, `fixture.md`), private/internal/test-local identifiers | No escalation. Follow the convention; convening the council here violates `council.md` §3. |
