...
| **3-PR2 — AspNetCore** | Not started | none yet | — | 3-PR1 is now merged, so this is unblocked; 1d dependency already satisfied; decision D3 gates its W7 only. Note that Phase 3's acceptance criteria and DoD still carry Rule14 and the `unit-test.md` §5.1 addendum, both deferred out of 3-PR1 by the docs/ restriction — they are outstanding at the phase level, not lost. |
| **4-mediatr** | Not started | none yet | — | Hard-depends on 3-PR1 merged (soft dependency per Plan 00 §10.1 note — do not reorder without a plan edit); that dependency is now satisfied |
...
| **5-B** (identifiers) | **Core/Must/Guard/Fluent all done and verified 100%/100% both TFMs.** 10 sub-areas: ULID, GUID version (numeric+string), JWT (structural shape, not signature verification), SemVer 2.0.0, cron-expression (5-field, ranges/steps/lists/named values), MAC address, media type, regex-pattern validity, Base64Url, UTF-8 byte-sequence. **Only DataAnnotations remains, then close-out.** A `TimeSpan`/nested-partial-class name collision was caught and fixed by the orchestrator's independent rebuild before it reached a Must-layer commit — see the process memory note on always rebuilding before trusting a "builds clean" claim. | `feature/rules-identifiers` (open) | — | Branch name matches plan; worktree/branch naming held for once. Sizing estimate (2-3 sessions) held roughly true given the sub-area count. |
| **5-C** (Unicode) | **Core/Must/Guard/Fluent all done and verified 100%/100% both TFMs; DataAnnotations 2 of 4 sub-areas done (BOM, UTF-16), Normalization + grapheme counts in flight.** 4 sub-areas total: byte-order mark detection, well-formed UTF-16 (surrogate-pair validity), Unicode normalization-form check (the flagged globalization-invariant-mode row — handled by making the null/undefined-form/malformed-UTF-16 checks mode-independent per the rule's own XML doc, rather than needing a runtime-mode-specific test), grapheme-cluster count (8 clauses). A `CS0411` generic-inference failure on `EnumRules.IsDefined<TEnum>(TEnum? value)` (called with a non-nullable enum) was caught and fixed by the orchestrator's independent rebuild before it reached a commit. | `feature/rules-unicode` (open) | — | Once DataAnnotations closes out, this unit is ready for close-out/merge. |
