...
| **5-E** (temporal + clock injection) | **In progress — Core/Must/Guard layers fully complete and coverage-clean.** Core clock injection fully done. Must layer: `MinimumAge` clause + full `TimeProvider?` sweep across all five temporal families + calendar predicates on DateOnly/DateTimeOffset — all tested (`b5b2641`…`ef00481`, MustClauses 3026/3026 both TFMs). Guard layer: `TimeProvider?` threading, `BelowMinimumAge`, and calendar predicates on DateOnly/DateTimeOffset — all tested (`dca298f`…`ae0f094`, GuardClauses 2677/2677 both TFMs). **Independently verified `-Scope Core`/`MustClauses`/`GuardClauses` -Enforce100 100%/100% line+branch, both TFMs, six separate runs (Core 4343/4343 lines + 2955/2955 branches; MustClauses 3773/3773 + 1326/1326; GuardClauses 3500/3500 + 1166/1166) — the Core/Must/Guard portion of Batch E is done.** Remaining: the entire Fluent and DataAnnotations layers (DA resolves `TimeProvider` from `ValidationContext`); their own coverage gate once they exist. `FixedTimeProvider` clock test double shipped in `tests/PineGuard.Testing/`. | `feature/rules-temporal` (open) | — | Two decisions applied per the 2026-08-30 finish-everything directive rather than paused on: **(1)** `Microsoft.Bcl.TimeProvider` 10.0.11 added as a `Directory.Packages.props` dependency (netstandard2.1-only `PackageReference` on `PineGuard.Core`) to unblock `DateTimeRules`/`DateTimeOffsetRules`/their string counterparts, which otherwise can't reference `TimeProvider` pre-net8 — official Microsoft BCL polyfill, same tier as this program's other Microsoft.Extensions.* pins, reversible if the owner objects. **(2)** Plan 05's minimum-age row contradicts itself (states `today.AddYears(-years) >= value` as the boundary, then glosses it with leap-day behavior that formula doesn't actually produce — `value.AddYears(+years) <= today` does). Shipped the literal formula (matches .NET's dominant age-calculation idiom, is the conservative reading for a permission gate), documented the actual 1-March-not-28-Feb consequence in the `HasMinimumAge` XML `<remarks>`, pinned with a named test. Both are one-line-reversible if the owner prefers otherwise — flag before Must/Guard/Fluent/DataAnnotations build on the semantic. |
...
| **6 — Analyzers** | **In progress — all six planned diagnostics implemented, tested, and coverage-clean.** `PineGuard.Analyzers`/`PineGuard.Analyzers.CodeFixes` (netstandard2.0) onboarded into the solution/CI. PG1001 (`Guard.Against.Null`), PG1002 (`NullOrWhiteSpace`), PG1003 (`NullOrEmpty`), PG1004 (`OutOfRange`), PG2001 (discarded `MustResult<T>`), PG2002 (discarded `MustValidationResult`) all done end to end — analyzer, code fix(es), fix-all, tests (`b2fb9d6`…`119eb66`). Shared coverage-scope registry (`tools/.shared/dotnet-projects.ps1`) generalized to support a two-source-project scope (`6fba593`). **Independently verified: `-Scope Analyzers -Enforce100` 100%/100% line+branch, both TFMs (434/434 lines, 146/146 branches, 12 classes); 118/118 tests both TFMs.** Remaining: README before/after blocks for PG1001-PG1004 (PG2001/PG2002 already have theirs), `.nupkg` layout verification (both DLLs under `analyzers/dotnet/cs`, no `lib/` folder, `developmentDependency` true), then close-out/merge. | `feature/analyzers` (open) | — | Deliberately run last (Plan 00 §3.2); no blockers. |
