...
| **5-E** (temporal + clock injection) | **In progress — Core clock injection fully done (DateOnly/DateTime/DateTimeOffset + string counterparts, `MinimumAge` added to `DateOnlyRules`/`DateTimeRules`). Must layer: `MinimumAge` clause added AND tested on DateOnly/DateTime/string-DateOnly (`b5b2641`, `c1cfd8a`, `02c73b5`, `09a3e2a`); full `TimeProvider?` sweep done across `Past`/`PastOrPresent`/`Future`/`FutureOrPresent`/`WithinDaysFromNow`/`NotWithinDaysFromNow` on all five temporal Must-clause families, with GuardClauses' positional call sites fixed to compile against the new signatures (`2250d96`).** Remaining: Guard's own `timeProvider` parameter + its `MinimumAge`/`BelowMinimumAge` wrappers; calendar-predicate (`Weekday`/`Weekend`/`FirstDayOfMonth`/`LastDayOfMonth`) Must/Guard wrappers for DateOnly/DateTimeOffset (Core rules already exist); the entire Fluent and DataAnnotations layers (DA resolves `TimeProvider` from `ValidationContext`); coverage gate. `FixedTimeProvider` clock test double shipped in `tests/PineGuard.Testing/`. | `feature/rules-temporal` (open) | — | Two decisions applied per the 2026-08-30 finish-everything directive rather than paused on: **(1)** `Microsoft.Bcl.TimeProvider` 10.0.11 added as a `Directory.Packages.props` dependency (netstandard2.1-only `PackageReference` on `PineGuard.Core`) to unblock `DateTimeRules`/`DateTimeOffsetRules`/their string counterparts, which otherwise can't reference `TimeProvider` pre-net8 — official Microsoft BCL polyfill, same tier as this program's other Microsoft.Extensions.* pins, reversible if the owner objects. **(2)** Plan 05's minimum-age row contradicts itself (states `today.AddYears(-years) >= value` as the boundary, then glosses it with leap-day behavior that formula doesn't actually produce — `value.AddYears(+years) <= today` does). Shipped the literal formula (matches .NET's dominant age-calculation idiom, is the conservative reading for a permission gate), documented the actual 1-March-not-28-Feb consequence in the `HasMinimumAge` XML `<remarks>`, pinned with a named test. Both are one-line-reversible if the owner prefers otherwise — flag before Must/Guard/Fluent/DataAnnotations build on the semantic. |
...
| **6 — Analyzers** | **In progress — worktree/branch created, dispatched.** | `feature/analyzers` (open) | — | Deliberately run last (Plan 00 §3.2); no blockers. This is now the only unit and, once 3-PR2/5-E close, the program's last remaining phase. |
