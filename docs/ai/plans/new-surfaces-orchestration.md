...
| **5-E** (temporal + clock injection) | **In progress — Core clock injection: DateOnly done (`IsInPast`/`IsInFuture`/`HasMinimumAge` + string counterparts), DateTime done (`IsInPast`/`IsInFuture`/`IsWithinDaysFromNow`/`HasMinimumAge`, `c71b0a6`, 4922/4922 tests both TFMs). Remaining Core work: `DateTimeOffsetRules.IsInPast`/`IsInFuture` and `StringRules.DateTimeOffset.IsInPast`/`IsInFuture` still read `DateTimeOffset.UtcNow` directly (not yet `TimeProvider`-injected — no `IsWithinDaysFromNow`/`HasMinimumAge` needed on that type per the plan's concept table). Then the full Must/Guard/Fluent/DataAnnotations parity sweep across all temporal families is untouched.** `FixedTimeProvider` clock test double shipped in `tests/PineGuard.Testing/`. Calendar predicates on DateOnly/DateTimeOffset also done. | `feature/rules-temporal` (open) | — | Two decisions applied per the 2026-08-30 finish-everything directive rather than paused on: **(1)** `Microsoft.Bcl.TimeProvider` 10.0.11 added as a `Directory.Packages.props` dependency (netstandard2.1-only `PackageReference` on `PineGuard.Core`) to unblock `DateTimeRules`/`DateTimeOffsetRules`/their string counterparts, which otherwise can't reference `TimeProvider` pre-net8 — official Microsoft BCL polyfill, same tier as this program's other Microsoft.Extensions.* pins, reversible if the owner objects. **(2)** Plan 05's minimum-age row contradicts itself (states `today.AddYears(-years) >= value` as the boundary, then glosses it with leap-day behavior that formula doesn't actually produce — `value.AddYears(+years) <= today` does). Shipped the literal formula (matches .NET's dominant age-calculation idiom, is the conservative reading for a permission gate), documented the actual 1-March-not-28-Feb consequence in the `HasMinimumAge` XML `<remarks>`, pinned with a named test. Both are one-line-reversible if the owner prefers otherwise — flag before Must/Guard/Fluent/DataAnnotations build on the semantic. |
