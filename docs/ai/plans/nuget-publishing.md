...
Progress: **10 / 12 complete** (step 10 substantially done, two carry-overs).
...
- [x] **10. Post-publish verification** — substantially complete on `v0.1.0-alpha.4` (commit `8f2979a`, workflow run 24602769219). Catalog probes across all six packages confirm embedded `iconFile=pineguard-icon-128px.png`, embedded `readmeFile=README.md`, authors/license/projectUrl correct, and per-package descriptions live. Direct nupkg inspection confirms `lib/net10.0/`, `lib/net8.0/`, `lib/netstandard2.1/` present across the five production packages (Testing ships net8.0/net10.0 only, as designed). Per-package tag strategy landed and verified live via nuget.org catalog (see notes). Two carry-overs remain: Source Link step-into-source test (requires hands-on VS/Rider session on the maintainer workstation; not something a CI probe can cover), and icon transparency (source master `pineguard-logo-512px.png` is opaque gray-on-gray — confirmed via pixel sampling; alpha is correctly preserved through the 128px downscale, but there's no transparency to preserve). Nothing blocks cutting `v1.0.0` once coverage drift is resolved.
    ...
    - [ ] **CARRY-OVER:** Source Link step-into-source test — needs maintainer IDE session
    - [ ] **CARRY-OVER:** Icon transparency — requires re-exporting `docs/brand/pineguard-logo-512px.png` from the design source with a true alpha channel, then regenerating the 128px variant. Current 128px correctly preserves whatever alpha is in the source (PNG color type 6, RGBA); the source simply has none.
- [ ] **11. Cut `v1.0.0`** — once the alpha pipeline is proven. Blocked on: coverage drift (main runs 92.2% line / 87.1% branch against the `MIN_CODE_COVERAGE=100` CI gate; see step-9 notes), and optionally the two step-10 carry-overs.
