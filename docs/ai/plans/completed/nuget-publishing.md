...
Progress: **10 / 12 complete** (all carry-overs resolved on `v0.1.0-alpha.5`; step 11 unblocked).
...
- [x] **10. Post-publish verification** — complete on `v0.1.0-alpha.5` (commits `daf5524` coverage fix + `7e490b6` transparent icon). Catalog probes across all six packages confirm embedded `iconFile=pineguard-logo-128px.png`, embedded `readmeFile=README.md`, authors/license/projectUrl correct, and per-package descriptions live. Direct nupkg inspection confirms `lib/net10.0/`, `lib/net8.0/`, `lib/netstandard2.1/` present across the five production packages (Testing ships net8.0/net10.0 only, as designed). Per-package tag strategy landed and verified live via nuget.org catalog (see notes). Source Link step-into-source verified on VS 2026 against `v0.1.0-alpha.5` — F11 on `EmailRules.IsEmail` opened the real `EmailRules.cs` from `github.com/stevomccormack/PineGuard` with full `/// <summary>` XML docs and original whitespace intact, not a decompiled stub. Icon transparency resolved by luminance chroma-keying the 128px downscale against the master's baked-in grey/white background — new `pineguard-logo-128px.png` (11.7 KB, 12271 transparent + 242 feathered + 3871 opaque pixels) renders cleanly against any nuget.org theme.
    - [x] Icon filename + README filename embedded correctly in every package (catalog-confirmed)
    - [x] Per-package tags live on nuget.org (alpha.4 catalog probe)
    - [x] net10.0 served at restore — `lib/net10.0/*.dll` present in every production nupkg (the absence from the header pill row is gallery compatibility-floor rendering, not a missing asset; see notes)
    - [x] Source Link step-into-source verified on VS 2026 (alpha.5) — real `.cs` with XML docs opens on F11
    - [x] Icon transparency — `pineguard-logo-128px.png` generated via `.etc/powershell/resize-icon.ps1` luminance chroma-key (strips the master's opaque grey/white bg during bicubic downscale); replaces `pineguard-icon-128px.png` and renames to match the `pineguard-logo-*px.png` convention across all six csprojs + `Directory.Build.props`
- [ ] **11. Cut `v1.0.0`** — once the alpha pipeline is proven. Coverage drift resolved: CI now 100% line / 100% branch (commit `daf5524` wired `--settings tools/code-coverage/coverlet.runsettings` into the CI test step and excluded `[*]System.Text.RegularExpressions.Generated.*` from instrumentation — those GeneratedRegex DFA state machines were inflating the uncovered line count). No outstanding blockers.
...
- **Per-package tag strategy overhaul (alpha.4).** The initial tag list inherited by every package from `Directory.Build.props` was `validation;guard;must;fluent;data-annotations;dotnet` — a flat repeat of PineGuard's own submodule names on six identical packages, offering zero discoverability beyond "found the PineGuard family." Redesigned in commit `8f2979a` around two principles: every tag must be *true* for the package it's on, and every tag must be a *plausible search query* a developer would actually type. Shape: a nine-tag common base (`validation validator input-validation model-validation validation-rules defensive-programming owasp csharp dotnet`) applied via `Directory.Build.props` across Core, Must, Guard, Fluent, DataAnnotations; each of the five then appends package-specific terms in its own `.csproj` by composing `<PackageTags>$(PackageTags);extras</PackageTags>`. Core adds `parsing;regex;format-validation`; Must adds `must-clauses;result-type;railway-oriented;functional-error-handling`; Guard adds `guard-clauses;argument-validation;argumentexception;null-check;fail-fast;ddd;clean-architecture;domain-driven-design`; Fluent adds `fluentvalidation;fluent-validation;rule-builder;abstractvalidator`; DA adds `data-annotations;validationattribute;mvc;blazor;model-binding`. Testing opts out of the base entirely with a narrow `xunit;testing;test-fixtures;validation-testing;pineguard` (its audience is test authors, not the application-validation persona). Framework identifiers (`net8`, `net10`, `netstandard2.1`) deliberately excluded — nuget.org's framework filter reads from `<TargetFrameworks>`, not tags, so listing them as keywords clutters the tag row without improving filterability. `ddd`/`domain-driven-design` appear only on Guard (textbook DDD boundary-validation pattern); dropped from Must because the result-type framing is Railway-Oriented Programming, not canonical DDD. `xss-prevention`/`injection-prevention` dropped from Core as overclaiming — Core validates shape/format, doesn't transform (`owasp` by itself is honest). `fail-fast` moved to Guard only (Must/Fluent/DA are collect-then-report, not fail-fast). Catalog probe on every package confirms the new tags are live.
- **Publish helper script `publish-release.ps1`.** `publish.yml` triggers on `release: published`, so cutting a release is the entry point to the whole publish flow. Encoded as `.etc/powershell/publish-release.ps1` (gitignored, local tooling): takes `-Version 0.1.0-alpha.4` (with or without leading `v`), auto-detects prerelease from `-alpha|-beta|-rc` and passes `--prerelease --latest=false` to `gh release create` so alphas don't displace the stable "Latest" pin, pre-flights on branch (main), working tree cleanliness, and tag availability (both local and remote), and optionally polls for the triggered workflow run and tails it via `gh run watch` when `-Watch` is passed. End-to-end ran in 1m37s for `v0.1.0-alpha.4`. Deliberately does *not* add `workflow_dispatch` to `publish.yml` itself — the script creates a real release so MinVer reads a real tag; a dispatch-only path would have to synthesize the version differently and doesn't match how releases are tracked.
- **net10.0 pill behaviour is by design, not a gap.** nuget.org's package-page header pill row shows *compatibility floors* — the minimum TFM of each compat family the package serves. For a package that ships `lib/netstandard2.1/`, `lib/net8.0/`, and `lib/net10.0/`, the gallery picks `.NET Standard 2.1` (floor for the netstandard family) and `.NET 8.0` (floor for the modern .NET family) because `net10.0` is forward-compatible with `net8.0` — a net10 consumer still restores `lib/net10.0/` via standard TFM precedence. The "Frameworks" tab *does* list every shipped TFM explicitly. Verified the packaging is correct by unzipping each `.nupkg` and confirming the `lib/net10.0/` folder is present — no fix needed; the question was misread initially as a display gap but is actually intentional terseness on the header.
- **Icon transparency — source master, not the downscale.** The 128px icon shipped in every package is honest about its content: PNG color type 6 (RGBA), 8-bit per channel, alpha channel fully present. Corner-pixel sampling via `System.Drawing.Bitmap.GetPixel` shows partial alpha at the extreme edges (A=219–237 from bicubic resampling) but near-white opaque gray (R≈220, G≈220, B≈220, A=255) across the tile. Sampling the same corners on the 512px master `pineguard-logo-512px.png` shows the *source* has opaque gray background too (A=255 everywhere). The downscale preserved alpha correctly; there's simply no transparency in the master to preserve. Decision: leave the 128px as-is and carry forward a task to re-export the 512px master with a true alpha channel from the design tool, then regenerate the 128px variant — that ships in the next metadata-touching release. Color-keying the gray to transparent was considered and rejected as a fragile band-aid that leaves halo artifacts on anti-aliased logo edges.
