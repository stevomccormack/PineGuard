...
- All tests are `[Theory]` + `TheoryData` (never `[Fact]`) with a paired `*TestData.cs` file — enforced by the `test-files` audit rule, which CI gates ([`.github/workflows/ci.yml`](../../../.github/workflows/ci.yml)).
...
- [ ] **Audit**: run [`/audit-cli`](audit-cli.md) — at minimum the `test-files` CI gate, plus `layer-parity` and `ordering`, since a slice adds methods to every layer.
