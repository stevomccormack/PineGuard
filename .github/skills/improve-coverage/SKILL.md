---
name: improve-coverage
description: Analyze PineGuard coverage gaps and fill them by following the canonical Brain workflow.
---
# Skill: Improve Coverage

## Load First

Read these files before changing tests:
1. [spec.md](../../../docs/ai/specs/spec.md)
2. [coverage.md](../../../docs/ai/specs/testing/coverage.md)
3. [unit-test.md](../../../docs/ai/specs/testing/unit-test.md)
4. [improve-code-coverage/SKILL.md](../../../docs/ai/skills/improve-coverage/SKILL.md)
5. [coverage-analyst memory](../../../docs/ai/memory/coverage-analyst.md)
6. [test-writer memory](../../../docs/ai/memory/test-writer.md)

## Execute

Follow the canonical recipe in [docs/ai/skills/improve-coverage/SKILL.md](../../../docs/ai/skills/improve-coverage/SKILL.md) exactly.

## Verify

- Use xplat coverage for the final result.
- Stop only when the requested scope reaches the expected target.
- Keep the Brain as the source of truth.
