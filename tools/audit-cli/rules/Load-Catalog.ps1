...
function Get-PineGuardAuditRule-Catalog {
    ...
    @(
        ...
        & $rule -Id 'Rule11' -Name 'Doc Links' -Description 'Resolves docs/ai, markdown, script and front-matter (template/parent/dependencies) path references in tracked .md files and .vscode/tasks.json.' -Script 'Test-Rule11-DocLinks.ps1' -OutputPath 'artifacts/audit/Rule11-doc-links.txt' -UsesFailOnFindings
        & $rule -Id 'Rule12' -Name 'Adapter Surface Parity' -Description 'Checks command parity between docs/ai/agents and the full adapter surfaces declared in docs/ai/meta/adapter-surfaces.md, honouring its declared subset policies.' -Script 'Test-Rule12-AdapterParity.ps1' -OutputPath 'artifacts/audit/Rule12-adapter-surface-parity.txt' -UsesFailOnFindings
        & $rule -Id 'Rule13' -Name 'Must Codes' -Description 'Validates the MustCodes error-code catalogue against the clause tree: every clause passes exactly one code, every code is used, no hardcoded code literals outside Codes/, clause files stay within their mapped domain, and Codes/ stays a dependency-free leaf.' -Script 'Test-Rule13-MustCodes.ps1' -OutputPath 'artifacts/audit/Rule13-must-codes.txt' -UsesFailOnFindings
