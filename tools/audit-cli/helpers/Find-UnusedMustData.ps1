...
[CmdletBinding()]
param(
    [ValidateNotNullOrEmpty()] [string]$AuditRuleId = 'Rule05',
    [string]$RepoRoot = '',
    [string]$OutputPath = '',
    [ValidateSet('Debug', 'Release')] [string]$Configuration = 'Release',
    [string]$MustProject = 'src/PineGuard.MustClauses/PineGuard.MustClauses.csproj',
    [string]$ScanRoot = 'src/PineGuard.DataAnnotations',
    [string]$ArtifactsSubdir = 'artifacts/audit',
    [string]$OutputFileName = 'find-unused-must-with-data-annotations.txt',
...
$tempPath = Join-Path ([System.IO.Path]::GetTempPath()) ("PineGuard-Audit-Must-DA-" + [System.Guid]::NewGuid().ToString('N'))
...
$regex = [regex]::new('(?x)\bMust\s*\.\s*Be\s*\.\s*([A-Za-z_][A-Za-z0-9_]*)\s*(?:<[^()]*>)?\s*\(', [System.Text.RegularExpressions.RegexOptions]::Compiled)
...
# 6. Report
$reportLines = @(
    "AuditRule: $AuditRuleId - MustClauses \u2192 DataAnnotations mapping (usage scan)",
...
if ($unusedNames.Count -gt 0) {
    $reportLines += "UNUSED MUST CLAUSES (Not used in DataAnnotations):"
    ...
    $reportLines += "All MustClauses public extensions are used in DataAnnotations."
}
...
if ($unusedNames.Count -eq 0) {
    Write-Host 'MustClauses → DataAnnotations usage: PASS' -ForegroundColor Green
    exit 0
}

Write-Host ("MustClauses → DataAnnotations usage: FAIL ({0} unused Must method(s))" -f $unusedNames.Count) -ForegroundColor Red
