...
public sealed class GuardStringTimeSpanClausesTests(ITestOutputHelper output) : BaseGuardUnitTest(output)
{
    ...
    { var value = tc.Value.value; var result = AssertResult(tc, () => Guard.Against.NotDurationBetween(value, tc.Value.min, tc.Value.max, tc.Value.inclusion)); if (tc.Expected.IsValid && value is not null) Assert.Equal(TimeSpan.Parse(value), result); }
    ...
    { var value = tc.Value.value; var result = AssertResult(tc, () => Guard.Against.DurationBetween(value, tc.Value.min, tc.Value.max, tc.Value.inclusion)); if (tc.Expected.IsValid && value is not null) Assert.Equal(TimeSpan.Parse(value), result); }
    ...
    { var value = tc.Value.value; var result = AssertResult(tc, () => Guard.Against.LessThan(value, tc.Value.threshold, tc.Value.inclusion)); if (tc.Expected.IsValid && value is not null) Assert.Equal(TimeSpan.Parse(value), result); }
    ...
    { var value = tc.Value.value; var result = AssertResult(tc, () => Guard.Against.GreaterThan(value, tc.Value.threshold, tc.Value.inclusion)); if (tc.Expected.IsValid && value is not null) Assert.Equal(TimeSpan.Parse(value), result); }
}
