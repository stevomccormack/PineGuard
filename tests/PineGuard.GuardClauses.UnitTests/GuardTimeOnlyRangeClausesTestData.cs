...
public static class GuardTimeOnlyRangeClausesTestData
{
    public static class NotChronological
    {
        public static TheoryData<GuardCase<(TimeOnlyRange range, Inclusion inclusion)>> ValidCases =>
            F.IsChronological.NonNullValidScenarios.ToGuardCases(_ => new GuardExpected(true));
        public static TheoryData<GuardCase<(TimeOnlyRange range, Inclusion inclusion)>> InvalidCases =>
    ...
    {
        public static TheoryData<GuardCase<(TimeOnlyRange range1, TimeOnlyRange range2, Inclusion inclusion)>> ValidCases =>
            F.IsOverlapping.NonNullInvalidScenarios.ToGuardCases(_ => new GuardExpected(true));
        public static TheoryData<GuardCase<(TimeOnlyRange range1, TimeOnlyRange range2, Inclusion inclusion)>> InvalidCases =>
    ...
    {
        public static TheoryData<GuardCase<(TimeOnlyRange range, TimeOnly value, Inclusion inclusion)>> ValidCases =>
            F.Contains.NonNullValidScenarios.ToGuardCases(_ => new GuardExpected(true));
        public static TheoryData<GuardCase<(TimeOnlyRange range, TimeOnly value, Inclusion inclusion)>> InvalidCases =>
