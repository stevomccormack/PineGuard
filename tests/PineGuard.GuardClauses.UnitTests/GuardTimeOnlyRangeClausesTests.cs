...
using TD = PineGuard.GuardClauses.UnitTests.GuardTimeOnlyRangeClausesTestData;

namespace PineGuard.GuardClauses.UnitTests;

public sealed class GuardTimeOnlyRangeClausesTests(ITestOutputHelper output) : BaseGuardUnitTest(output)
{
    [Theory]
    ...
    public void NotChronological_BehavesAsExpected(GuardCase<(TimeOnlyRange range, Inclusion inclusion)> tc)
    {
    ...
    public void Overlapping_BehavesAsExpected(GuardCase<(TimeOnlyRange range1, TimeOnlyRange range2, Inclusion inclusion)> tc)
    {
    ...
    public void NotOverlapping_BehavesAsExpected(GuardCase<(TimeOnlyRange range1, TimeOnlyRange range2, Inclusion inclusion)> tc)
    {
        var range1 = tc.Value.range1;
    ...
    public void NotContains_BehavesAsExpected(GuardCase<(TimeOnlyRange range, TimeOnly value, Inclusion inclusion)> tc)
    {
        var range = tc.Value.range;
    ...
    public void Contains_BehavesAsExpected(GuardCase<(TimeOnlyRange range, TimeOnly value, Inclusion inclusion)> tc)
    {
        var range = tc.Value.range;
