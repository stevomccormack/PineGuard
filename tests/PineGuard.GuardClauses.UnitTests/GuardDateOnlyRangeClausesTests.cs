...
using TD = PineGuard.GuardClauses.UnitTests.GuardDateOnlyRangeClausesTestData;

namespace PineGuard.GuardClauses.UnitTests;

public sealed class GuardDateOnlyRangeClausesTests(ITestOutputHelper output) : BaseGuardUnitTest(output)
{
    [Theory]
    ...
    public void NotChronological_BehavesAsExpected(GuardCase<(DateOnlyRange range, Inclusion inclusion)> tc)
    {
    ...
    public void Overlapping_BehavesAsExpected(GuardCase<(DateOnlyRange range1, DateOnlyRange range2, Inclusion inclusion)> tc)
    {
    ...
    public void NotOverlapping_BehavesAsExpected(GuardCase<(DateOnlyRange range1, DateOnlyRange range2, Inclusion inclusion)> tc)
    {
        var range1 = tc.Value.range1;
    ...
    public void NotContains_BehavesAsExpected(GuardCase<(DateOnlyRange range, DateOnly value, Inclusion inclusion)> tc)
    {
        var range = tc.Value.range;
    ...
    public void Contains_BehavesAsExpected(GuardCase<(DateOnlyRange range, DateOnly value, Inclusion inclusion)> tc)
    {
        var range = tc.Value.range;
