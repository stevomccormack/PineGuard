...
public sealed class GuardTimeOnlyClausesTests(ITestOutputHelper output) : BaseGuardUnitTest(output)
{
    ...
    { var start = tc.Value.start; var result = AssertResult(tc, () => Guard.Against.Chronological(start, tc.Value.end, tc.Value.inclusion)); AssertCustomMessage(tc, () => Guard.Against.Chronological(start, tc.Value.end, tc.Value.inclusion, message: CustomMessage)); if (tc.Expected.IsValid) Assert.Equal(start, result); }
