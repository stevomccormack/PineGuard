...
public sealed class RuleComparisonTests : BaseUnitTest
{
    ...
    {
        // Act
        var greater = InvokeGenericStatic<bool>("IsGreaterThan", testCase.Value.Value, testCase.Value.Boundary,
            testCase.Value.Inclusion);
        var less = InvokeGenericStatic<bool>("IsLessThan", testCase.Value.Value, testCase.Value.Boundary,
            testCase.Value.Inclusion);

        // Assert
        Assert.Equal(testCase.Expected.ExpectedGreaterThan, greater);
        Assert.Equal(testCase.Expected.ExpectedLessThan, less);
    }
    ...
    public void IsGreaterThan_Throws_ForInvalidInclusion(RuleComparisonTestData.Boundaries.InvalidCase testCase)
    {
        ...
        // Act
        var ex = Assert.Throws<TargetInvocationException>(() =>
            InvokeGenericStatic<bool>("IsGreaterThan", invalidCase.Value.Value, invalidCase.Value.Boundary,
                invalidCase.Value.Inclusion));
    ...
    public void IsLessThan_Throws_ForInvalidInclusion(RuleComparisonTestData.Boundaries.InvalidCase testCase)
    {
        ...
        // Act
        var ex = Assert.Throws<TargetInvocationException>(() =>
            InvokeGenericStatic<bool>("IsLessThan", invalidCase.Value.Value, invalidCase.Value.Boundary,
                invalidCase.Value.Inclusion));
