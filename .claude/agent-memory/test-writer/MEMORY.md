...
- [new-package-testdata-patterns.md](new-package-testdata-patterns.md) — Brand-new package w/ no PineGuard.Testing fixture yet: hand-built cases, enclosing-namespace visibility rules, DA attribute + ValidationContext service provider pattern, grounding dynamic messages via an existing spike test, concurrent-source-fix polling protocol
- [must-validator-layer-test-pattern.md](must-validator-layer-test-pattern.md) — Testing sealed `IMustValidator<T>` classes: `BaseMustValidationUnitTest`/`MustValidationCase`/`MustValidationExpected` (own "layer", not "(Other)"); non-generic surface via plain per-member Theory (no Func-wrapping needed); verify constructor null-checks are lazy not eager before asserting; get runtime ground truth via spike `[Fact]` before writing schema-engine Expected values
