...
public sealed partial record IsoCountry
{
    ...
    public IsoCountry(string alpha2Code, string alpha3Code, string numericCode, string name)
    {
        ...
        if (!Alpha2CodeRegex().IsMatch(alpha2Code))
            throw new ArgumentException($"{nameof(alpha2Code).TitleCase()} must be alphabetic with exact length of {Alpha2CodeExactLength} characters.", nameof(alpha2Code));
    ...
    public static IsoCountry Parse(string? value)
    {
        ...
        throw new FormatException($"{nameof(value).TitleCase()} must be an ISO 3166-1 alpha-2, alpha-3, or numeric country code.");
    }
