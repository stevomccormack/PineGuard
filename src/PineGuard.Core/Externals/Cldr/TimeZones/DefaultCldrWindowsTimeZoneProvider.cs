...
public sealed class DefaultCldrWindowsTimeZoneProvider : ICldrWindowsTimeZoneProvider
{
    ...
    {
        return BuildWindowsIdByIanaIdAndTerritoryIndex(DefaultCldrWindowsTimeZoneData.IanaTimeZoneIdsByWindowsId);
    }

    internal static FrozenDictionary<string, FrozenDictionary<string, string>> BuildWindowsIdByIanaIdAndTerritoryIndex(
        IEnumerable<KeyValuePair<string, FrozenDictionary<string, string[]>>> windowsToTerritoryToIana)
    {
    ...
    {
        return TryGetIanaTimeZoneIds(IanaTimeZoneIdsByWindowsIdIndex.Value, windowsTimeZoneId, territory, out ianaTimeZoneIds);
    }

    internal static bool TryGetIanaTimeZoneIds(
        FrozenDictionary<string, FrozenDictionary<string, string[]>> index,
        string? windowsTimeZoneId,
        string? territory,
        out IReadOnlyCollection<string> ianaTimeZoneIds)
    {
        ianaTimeZoneIds = [];
        ...
        if (!index.TryGetValue(winId, out var byTerritory))
            return false;
    ...
    {
        return TryGetWindowsTimeZoneId(WindowsIdByIanaIdAndTerritoryIndex.Value, ianaTimeZoneId, territory, out windowsTimeZoneId);
    }

    internal static bool TryGetWindowsTimeZoneId(
        FrozenDictionary<string, FrozenDictionary<string, string>> index,
        ...
        out string windowsTimeZoneId)
    {
        ...
        if (!index.TryGetValue(ianaId, out var byTerritory))
            return false;
