...
public static partial class StringRules
{
    ...
    /// </summary>
    /// <param name="value">The value to validate. If <see langword="null"/> or empty, returns <see langword="false"/>. Leading and trailing whitespace is trimmed before validation.</param>
    ...
    /// <param name="value">The value to validate. If <see langword="null"/> or empty, returns <see langword="false"/>. Leading and trailing whitespace is trimmed before validation.</param>
    /// <returns>
    /// <see langword="true"/> if every letter in <paramref name="value"/> is uppercase using invariant comparison; otherwise, <see langword="false"/>.
    /// A string with no letter characters at all (e.g., <c>"123"</c>) returns <see langword="true"/> vacuously, since there is no lowercase letter to disprove the claim.
    /// </returns>
    public static bool IsUpperInvariant(string? value) =>
        StringUtility.TryGetTrimmed(value, out var trimmed) && trimmed.Where(char.IsLetter).All(ch => !char.IsLower(ch));
    ...
    /// <param name="value">The value to validate. If <see langword="null"/> or empty, returns <see langword="false"/>. Leading and trailing whitespace is trimmed before validation.</param>
    /// <returns>
    /// <see langword="true"/> if every letter in <paramref name="value"/> is lowercase using invariant comparison; otherwise, <see langword="false"/>.
    /// A string with no letter characters at all (e.g., <c>"123"</c>) returns <see langword="true"/> vacuously, since there is no uppercase letter to disprove the claim.
    /// </returns>
