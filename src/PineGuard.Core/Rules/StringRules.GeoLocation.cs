...
public static partial class StringRules
{
    ...
    public static class GeoLocation
    {
        ...
        public static bool IsLatitude(string? latitude) => StringUtility.NumberTypes.TryParseDouble(latitude, out var parsed) && GeoLocationRules.IsLatitude(parsed);
        ...
        public static bool IsLongitude(string? longitude) =>
            StringUtility.NumberTypes.TryParseDouble(longitude, out var parsed) && GeoLocationRules.IsLongitude(parsed);
        ...
        {
            if (!StringUtility.NumberTypes.TryParseDouble(latitude, out var lat))
                return false;

            return StringUtility.NumberTypes.TryParseDouble(longitude, out var lon) && GeoLocationRules.IsGeoLocation(lat, lon);
        }
    }
}
#endif
