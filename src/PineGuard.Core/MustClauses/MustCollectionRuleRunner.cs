...
/// <summary>
/// Runs a single-argument <c>RuleForEach(x =&gt; x.Items, item =&gt; Must.Be.X(item))</c> rule against
/// every element, reporting failures at <c>Property[i]</c>. Skips a <see langword="null"/> collection;
/// enumerates exactly once.
/// </summary>
internal sealed class MustCollectionRuleRunner<T, TItem, TResult>(
    string propertyPath,
    Func<T, IEnumerable<TItem>?> accessor,
    Func<TItem, MustResult<TResult>> check)
