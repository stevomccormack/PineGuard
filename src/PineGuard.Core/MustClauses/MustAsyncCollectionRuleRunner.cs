...
/// <summary>
/// Runs a single-argument <c>RuleForEachAsync(x =&gt; x.Items, (item, ct) =&gt; …)</c> rule against every
/// element, reporting failures at <c>Property[i]</c>. Skips a <see langword="null"/> collection;
/// enumerates exactly once; awaits each element in order.
/// </summary>
internal sealed class MustAsyncCollectionRuleRunner<T, TItem, TResult>(
    string propertyPath,
    Func<T, IEnumerable<TItem>?> accessor,
    Func<TItem, CancellationToken, ValueTask<MustResult<TResult>>> check)
