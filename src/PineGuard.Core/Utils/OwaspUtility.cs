...
/// </summary>
/// <remarks>
/// <b>Null contract:</b> every method here treats <see langword="null"/> or empty input as
/// <b>risk-free</b> (returns <see langword="false"/>), since there is no content to match a risk pattern
/// against; most also treat whitespace-only input the same way, with one deliberate exception:
/// <see cref="ContainsCrLfRisk(string?)"/> matches CR/LF untrimmed, so a payload consisting entirely of
/// CR/LF whitespace (e.g. a bare <c>"\r\n"</c>) is still reported as risky &#8212; see its remarks. This
/// is the opposite convention from the safety predicates in <see cref="PineGuard.Rules.OwaspRules"/>,
/// where <c>Is*Safe(null)</c> returns <see langword="false"/> meaning "unsafe". The two are not
/// interchangeable: <c>!ContainsSqlInjectionRisk(value)</c> is <b>not</b> equivalent to
/// <see cref="PineGuard.Rules.OwaspRules.IsSqlInjectionSafe"/> for <see langword="null"/> or whitespace
/// input.
/// </remarks>
...
public static class OwaspUtility
{
    ...
    /// </summary>
    /// <param name="value">The value to check. If <see langword="null"/> or empty, returns <see langword="false"/>.</param>
    ...
    /// For the same reason the null/empty guard uses <see cref="string.IsNullOrEmpty(string?)"/> rather than
    /// <see cref="string.IsNullOrWhiteSpace(string?)"/>: CR and LF are themselves whitespace characters, so a
    /// whitespace-based guard would report "no risk" for a payload made entirely of CR/LF (e.g. a bare
    /// <c>"\r\n"</c>) &#8212; exactly the injection it exists to catch.
    /// </remarks>
    public static bool ContainsCrLfRisk(string? value) =>
        !string.IsNullOrEmpty(value) && OwaspRegex.HeaderInjection.CrLfRegex().IsMatch(value);
