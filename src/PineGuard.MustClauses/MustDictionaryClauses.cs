...
public static class MustDictionaryClauses
{
        ...
        [CallerArgumentExpression(nameof(dictionary))] string? paramName = null)
    {
        ...
        return MustResult<IDictionary<TKey, TValue>?>.FromBool(ok, messageTemplate, paramName, dictionary, dictionary);
    }
        ...
        [CallerArgumentExpression(nameof(dictionary))] string? paramName = null)
    {
        if (predicate is null)
            return MustResult<IDictionary<TKey, TValue>>.Fail(NullMessage, nameof(predicate), predicate);
