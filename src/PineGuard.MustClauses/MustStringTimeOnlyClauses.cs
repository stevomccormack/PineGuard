...
public static class MustStringTimeOnlyClauses
{
    ...
    {
        if (value is null)
            return MustResult<TimeOnly>.Fail(NullMessage, paramName, value);

        if (min > max)
            return MustResult<TimeOnly>.Fail("{paramName} must be less than or equal to " + nameof(max) + ".", nameof(min), min);
        ...
        if (!StringUtility.TimeOnly.TryParse(value, out var parsed, styles))
            return MustResult<TimeOnly>.FromBool(false, messageTemplate, paramName, value);

        var parsedValue = parsed.GetValueOrDefault();
        var ok = TimeOnlyRules.IsBetween(parsedValue, min, max, inclusion);
        return MustResult<TimeOnly>.FromBool(ok, messageTemplate, paramName, value, parsedValue);
    }
        ...
        [CallerArgumentExpression(nameof(value))] string? paramName = null)
    {
        ...
        if (reference is null)
            return MustResult<TimeOnly>.Fail(NullMessage, nameof(reference), reference);

        if (window < TimeSpan.Zero)
            return MustResult<TimeOnly>.Fail("{paramName} requires a non-negative time window.", nameof(window), window);
        ...
        return MustResult<TimeOnly>.FromBool(ok, messageTemplate, paramName, value, parsed);
    }
        ...
        [CallerArgumentExpression(nameof(value))] string? paramName = null)
    {
        ...
        if (precision is not null && !Enum.IsDefined(precision.Value))
            return MustResult<TimeOnly>.Fail(InvalidTimePrecisionMessage, nameof(precision), precision);
    ...
    {
        if (start is null)
            return MustResult<TimeOnly>.Fail(NullMessage, paramName, start);

        if (end is null)
            return MustResult<TimeOnly>.Fail(NullMessage, nameof(end), end);
        ...
        if (!StringUtility.TimeOnly.TryParse(start, out var parsedStart, styles))
            return MustResult<TimeOnly>.FromBool(false, messageTemplate, paramName, start);

        if (!StringUtility.TimeOnly.TryParse(end, out var parsedEnd, styles))
            return MustResult<TimeOnly>.FromBool(false, messageTemplate, nameof(end), end);
        ...
        return MustResult<TimeOnly>.FromBool(ok, messageTemplate, paramName, start, parsedStartValue);
    }
    ...
    {
        if (start1 is null)
            return MustResult<TimeOnly>.Fail(NullMessage, paramName, start1);

        if (end1 is null)
            return MustResult<TimeOnly>.Fail(NullMessage, nameof(end1), end1);

        if (start2 is null)
            return MustResult<TimeOnly>.Fail(NullMessage, nameof(start2), start2);

        if (end2 is null)
            return MustResult<TimeOnly>.Fail(NullMessage, nameof(end2), end2);

        const string messageTemplate = "{paramName} must be overlapping.";

        if (!StringUtility.TimeOnly.TryParse(start1, out var s1, styles))
            return MustResult<TimeOnly>.FromBool(false, messageTemplate, paramName, start1);

        if (!StringUtility.TimeOnly.TryParse(end1, out var e1, styles))
            return MustResult<TimeOnly>.FromBool(false, messageTemplate, nameof(end1), end1);

        if (!StringUtility.TimeOnly.TryParse(start2, out var s2, styles))
            return MustResult<TimeOnly>.FromBool(false, messageTemplate, nameof(start2), start2);

        if (!StringUtility.TimeOnly.TryParse(end2, out var e2, styles))
            return MustResult<TimeOnly>.FromBool(false, messageTemplate, nameof(end2), end2);
        ...
        var ok = TimeOnlyRules.IsOverlapping(
            ...
            inclusion);
        return MustResult<TimeOnly>.FromBool(ok, messageTemplate, paramName, start1, parsedStartValue);
    }
