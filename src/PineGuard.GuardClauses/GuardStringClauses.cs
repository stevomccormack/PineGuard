...
public static class GuardStringClauses
{
    ...
    /// <summary>
    /// Throws if <paramref name="value"/> is not a valid regular expression pattern.
    /// </summary>
    /// <param name="_">The <see cref="IGuardClause"/> entry point (used via <c>Guard.Against</c>).</param>
    /// <param name="value">The string to guard as a <see cref="Regex"/> pattern.</param>
    /// <param name="message">
    /// An optional custom error message. If <see langword="null"/>, uses the default message
    /// from <see cref="MustStringClauses.RegexPattern"/>.
    /// </param>
    /// <param name="exceptionCreator">
    /// An optional factory to create a custom exception. If <see langword="null"/>,
    /// throws <see cref="ArgumentException"/> via <see cref="GuardFailure.Throw"/>.
    ...
    /// <returns>The validated value of <paramref name="value"/> if the guard passes.</returns>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="value"/> cannot be compiled as a regular expression and no
    /// <paramref name="exceptionCreator"/> is provided.
    /// </exception>
    /// <remarks>
    /// This guard is the complement of <see cref="MustStringClauses.RegexPattern"/>:
    /// <c>Guard.Against.NotRegexPattern</c> passes when the value compiles as a
    /// <see cref="Regex"/> pattern. It is the guard to reach for before accepting a
    /// caller-supplied pattern — a passing guard says the syntax is legal, never that the
    /// pattern is efficient or safe to run against untrusted input.
    /// </remarks>
    /// <example>
    /// <code>
    /// Guard.Against.NotRegexPattern(userSuppliedFilter);
    /// </code>
    /// </example>
    /// <seealso cref="MustStringClauses.RegexPattern"/>
    public static string NotRegexPattern(this IGuardClause _,
        string? value,
        string? message = null,
        Func<Exception>? exceptionCreator = null,
        [CallerArgumentExpression(nameof(value))] string? paramName = null)
    {
        var result = Must.Be.RegexPattern(value, paramName); // Guard.Against.NotRegexPattern => Must.Be.RegexPattern (complement)
        if (result.Failed)
            GuardFailure.Throw(result, message, exceptionCreator);
